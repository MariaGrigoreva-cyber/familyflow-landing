...
function makePlainSvg({ text, number, total, title }) {
  const { fontSize, maxChars, lineHeight } = fontSettings(text, false);
  const lines = wrapText(text, maxChars);

  const textHeight = lines.length * lineHeight;
  const startY = Math.round(650 - textHeight / 2);
  ...
  <svg width="1080" height="1350" viewBox="0 0 1080 1350"
    ...
    <circle cx="1010" cy="180" r="205"
            fill="#B0502F" opacity="0.08"/>

    <circle cx="70" cy="1240" r="145"
            fill="#B0502F" opacity="0.05"/>
    ...
    <line x1="90" y1="155" x2="990" y2="155"
          stroke="#E8DCD3"
          stroke-width="2"/>
